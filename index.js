...
const archivesDirectory = config.archiveDirectoryName;
const mainDirectory = path.join(__dirname, archivesDirectory);
const archiveManager = new ArchiveManager({ mainDirectory });
...
app.get("/", (req, res) => {
	return res.render("index.hbs", {
		archives: archiveManager.getArchives().map((archive) => {
			...
			return {
				full: fs.pathExistsSync(path.join(dir, "full", "index.html"))
					? path.join(dir, "full", "index.html")
					: "",
				archiveorg: fs.pathExistsSync(path.join(dir, "archive.org.txt"))
					? fs.readFileSync(path.join(dir, "archive.org.txt"))
					: "",
				favicon: fs.pathExistsSync(path.join(dir, "favicon.ico"))
					? path.join(dir, "favicon.ico")
					: "",
				dom: fs.pathExistsSync(path.join(dir, "output.html"))
					? path.join(dir, "output.html")
					: "",
				pdf: fs.pathExistsSync(path.join(dir, "output.pdf"))
					? path.join(dir, "output.pdf")
					: "",
				screenshot: fs.pathExistsSync(path.join(dir, "output.png"))
...
app.get("/d/:id", function (req, res) {
	const archive = archiveManager.getArchivePath(req.params.id);
	...
	return zip
		.directory(
			path.join(__dirname, archivesDirectory, archive.folder, "full"),
			false
		)
		.finalize();
});
...
app.listen(config.serverPort, () =>
	console.log(`[@] ark running on ${config.serverPort}!`)
);
